...
class VulNewImage {
    public:
        ...
        static VulNewImage createDefaultWholeImageAllInOneSingleTime(vulB::VulDevice &vulDevice, std::variant<const std::string &,
                const std::vector<MipLevel> &> data, std::variant<KtxCompressionFormat, VkFormat> format, bool addSampler,
                InputDataType dataType, ImageType imageType);
        static VulNewImage createDefaultWholeImageAllInOne(vulB::VulDevice &vulDevice, std::variant<const std::string &,
                const std::vector<MipLevel> &> data, std::variant<KtxCompressionFormat, VkFormat> format, bool addSampler,
                InputDataType dataType, ImageType imageType, VkCommandBuffer cmdBuf);
        static VulNewImage createSamplerlessCustomImageAllInOneSingleTime(vulB::VulDevice &vulDevice,
                std::variant<const std::string &, const std::vector<MipLevel> &> data, std::variant<KtxCompressionFormat,
        ...
        static VulNewImage createSamplerlessCustomImageAllInOne(vulB::VulDevice &vulDevice, std::variant<const std::string &,
                const std::vector<MipLevel> &> data, std::variant<KtxCompressionFormat, VkFormat> format, InputDataType dataType,
        ...
        static VulNewImage createCustomImageAllInOneSingleTime(vulB::VulDevice &vulDevice,
                std::variant<const std::string &, const std::vector<MipLevel> &> data, std::variant<KtxCompressionFormat,
                VkFormat> format, InputDataType dataType, VkImageViewType type, VkImageLayout layout, VkImageUsageFlags usage,
                VkMemoryPropertyFlags memoryProperties, VkImageTiling tiling, VkFilter filter, VkSamplerAddressMode addressMode,
                float maxAnisotropy, VkBorderColor borderColor, float mipLodBias, float mipMinLod, float mipMaxLod);
        static VulNewImage createCustomImageAllInOne(vulB::VulDevice &vulDevice, std::variant<const std::string &,
                const std::vector<MipLevel> &> data, std::variant<KtxCompressionFormat, VkFormat> format, InputDataType dataType,
                VkImageViewType type, VkImageLayout layout, VkImageUsageFlags usage, VkMemoryPropertyFlags memoryProperties,
        ...
        void loadRawFromMemoryWhole(const std::vector<MipLevel> &mipLevels, VkFormat format);
        void loadRawFromMemory(const std::vector<MipLevel> &mipLevels, VkFormat format, uint32_t baseOutputMipLevel,
