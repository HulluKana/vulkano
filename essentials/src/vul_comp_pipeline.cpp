...
VulCompPipeline::VulCompPipeline(const std::vector<std::string> &shaderNames, const std::vector<VkDescriptorSetLayout> &setLayouts, vulB::VulDevice &device, uint32_t maxFramesInFlight) : m_vulDevice{device}
{
    ...
    for (size_t i = 0; i < shaderNames.size(); i++) {
        ...
        VulPipeline::createShaderModule(m_vulDevice, shaderNames[i], &shader);
