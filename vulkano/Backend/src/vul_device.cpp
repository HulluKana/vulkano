...
static VKAPI_ATTR VkBool32 VKAPI_CALL debugCallback(
    ...
    if (messageSeverity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_INFO_BIT_EXT) std::cerr << " info";
    if (messageSeverity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT) std::cerr << " error";
    if (messageSeverity == VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT) std::cerr << " verbose";
...
void VulDevice::createCommandPool() {
...
void VulDevice::createSurface() { window.createWindowSurface(instance, &surface_); }
...
SwapChainSupportDetails VulDevice::querySwapChainSupport(VkPhysicalDevice device) {
...
VkFormat VulDevice::findSupportedFormat(
...
uint32_t VulDevice::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void VulDevice::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
...
void VulDevice::copyBufferToImage(
...
void VulDevice::createImageWithInfo(
